...
;;; - bezier
;;;
;;; Each primitive has a variant that ends with * (e.g. point*) that can be
;;; called with simple-vectors instead of flat points.
...
(defun point* (v)
  "Draws a single point at V where V is a 2D simple-vector."
  (point (aref v 0) (aref v 1)))
...
(defun line* (p1 p2 &rest points)
  "Draws a line from P1 to P2, where each is a 2D simple-vector. More points can
be given."
  (let ((vertices (loop for p in points
                        collect (aref p 0)
                        collect (aref p 1))))
    (apply 'line (aref p1 0) (aref p1 1)
           (aref p2 0) (aref p2 1) vertices)))
...
(defun eq-tri* (v radius)
  "Draws an equilateral triangle with center V where V is a 2D simple-vector.
The length from the center to its points is RADIUS."
  (eq-tri (aref v 0) (aref v 1) radius))
...
(defun tri* (p1 p2 p3)
  "Draws a triangle with points P1, P2, P3, each being a 2D simple-vector."
  (tri (aref p1 0) (aref p1 1)
       (aref p2 0) (aref p2 1)
       (aref p3 0) (aref p3 1)))
...
(defun rect* (v w h)
  "Draws a rectangle with the top left corner at V, where V is a 2D
simple-vector."
  (rect (aref v 0) (aref v 1) w h))
...
(defun polygon* (v radius sides)
  "Draws a polygon at V where V is a 2D simple-vector."
  (polygon (aref v 0) (aref v 1) radius sides))
...
(defun circle* (v radius)
  "Draws a circle of size RADIUS at V where V is a 2D simple-vector."
  (circle (aref v 0) (aref v 1) radius))
